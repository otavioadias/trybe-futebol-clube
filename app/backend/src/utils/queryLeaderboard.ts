...
t.team_name AS name,
 ...
 ((SELECT 
    ...
    WHERE home_team_goals > away_team_goals AND home_team = t.id 
       AND in_progress = 0 
       OR away_team_goals > home_team_goals AND away_team = t.id AND in_progress = 0) * 3 )
       + ((SELECT 
    COUNT(*) 
       FROM TRYBE_FUTEBOL_CLUBE.matches 
       WHERE home_team_goals < away_team_goals AND home_team = t.id AND in_progress = 0 
          OR away_team_goals < home_team_goals AND away_team = t.id AND in_progress = 0) * 0)
          + ((SELECT 
 COUNT(*) 
       FROM TRYBE_FUTEBOL_CLUBE.matches 
    WHERE away_team_goals = home_team_goals AND home_team = t.id AND in_progress = 0 
       OR home_team_goals = away_team_goals AND away_team = t.id AND in_progress = 0) * 1))) 
       AS totalPoints,
  (SELECT 
 COUNT(*) 
     FROM TRYBE_FUTEBOL_CLUBE.matches
    WHERE (away_team = t.id AND in_progress = 0) 
       OR (home_team = t.id AND in_progress = 0)) AS totalGames,
  (SELECT 
 COUNT(*) 
     FROM TRYBE_FUTEBOL_CLUBE.matches 
    WHERE home_team_goals > away_team_goals AND home_team = t.id AND in_progress = 0 
       OR away_team_goals > home_team_goals AND away_team = t.id AND in_progress = 0) 
       AS totalVictories,
  (SELECT 
 COUNT(*) 
     FROM TRYBE_FUTEBOL_CLUBE.matches 
 ...
 COUNT(*) 
     FROM TRYBE_FUTEBOL_CLUBE.matches 
    WHERE away_team_goals = home_team_goals AND home_team = t.id AND in_progress = 0 
       OR home_team_goals = away_team_goals AND away_team = t.id AND in_progress = 0) 
       AS totalDraws,
  (SELECT 
 ...
 ((SELECT 
  COUNT(*) 
     ...
     WHERE home_team_goals < away_team_goals 
       AND home_team = t.id AND in_progress = 0 
        OR away_team_goals < home_team_goals AND away_team = t.id AND in_progress = 0) * 0)
+
  ((SELECT 
  COUNT(*) 
  ...
  COUNT(*) 
      FROM TRYBE_FUTEBOL_CLUBE.matches 
     WHERE away_team = t.id AND in_progress = 0 
        OR home_team = t.id AND in_progress = 0)) * 3)) * 100)) 
        AS efficiency
